 ...
 */
public class DelData extends Thread {
	...
	public DelData(Configuration conf) {
		this.conf = conf;
		this.tranDate = conf.tranDate;
		this.delDay = Integer.parseInt(conf.get("export.core.DelData"));
	}
	...
	public void run() {
		...
		String directory = PubFun.replaceDate(
				conf.get("export.core.dataDir.String"), dropDate);
			...
			for (File file : files) {
				if (file.getName().substring(0, 8).equals(dropDate)) {
					...
					if (file.getName().endsWith(".dat")
							...
							|| file.getName().endsWith(".dat.zip")
							|| file.getName().endsWith(".chk.zip")) {
						deleteFile(file);
					}
				}
			}
